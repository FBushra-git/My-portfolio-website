...
export default function Footer() {
  return (
    <footer className="mx-auto w-[min(1080px,calc(100%-32px))] border-t border-[var(--line)] py-8 text-[var(--muted)]">
      <div className="flex flex-col gap-5 !rounded-2xl border border-[var(--line)] bg-[color-mix(in_srgb,var(--surface)_72%,transparent)] p-6 shadow-[0_18px_50px_rgba(24,21,20,0.08)] backdrop-blur-xl md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-3" aria-label="Footer links">
          {footerLinks.map(({ label, href, Icon, external }) => (
            <a
              className="grid h-10 w-10 place-items-center !rounded-lg border border-[var(--line)] bg-[color-mix(in_srgb,var(--surface-strong)_52%,transparent)] text-[var(--text)] transition hover:border-[color-mix(in_srgb,var(--accent)_55%,var(--line))]"
              href={href}
              key={label}
